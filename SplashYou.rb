...
class Planetoftheapis
  ...
  def make_call
    @params = @params.to_json unless @meth.casecmp('get').zero? || @meth.casecmp('delete').zero?
    begin
...
def check_api_response(got)
  error = false
  if got['response'].key? 'message'
    error = true if got['response']['message'] =~ /Endpoint request timed out/
  end
  if got['response'].key? '_embedded'
    if got['response']['_embedded'].key? 'media-items'
...
@vids = grab_videos(10, @counter) # page size and page number
something_went_wrong if @vids['error'] == true
...
vid_array = @vids.sort_by { |_k, v| v['reach'] }.reverse
puts vid_array
